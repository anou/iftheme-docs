/*global wpml_tm_strings, Backbone, icl_ajxloaderimg, ajaxurl */
...
var WPMLTMDashboard = Backbone.View.extend({
    ...
    submit: function (e) {
        ...
        if (self.counts.duplicate > 0) {
            ...
            var duplication_ui = new PostDuplication(post_ids, langs, jQuery('#icl_dup_ovr_warn'));
            duplication_ui.sendFive();
        }
    },
...
var PostDuplication = Backbone.View.extend({
    ...
    duplicatedIDs: [],
    langs: '',
    initialize: function (posts, langs, element) {
        var self = this;
        self.posts = posts;
        self.langs = langs.join(',');
    ...
    sendFive: function () {
        var self = this;
        var posts_to_send = self.posts.splice(0, 5);
        var nonce = wpml_tm_strings.wpml_duplicate_dashboard_nonce;
        jQuery.ajax({
            type: "POST",
            url: ajaxurl,
            dataType: 'json',
            data: {
                ...
                duplicate_post_ids: posts_to_send.join(','),
                duplicate_target_languages: self.langs,
                _icl_nonce: nonce
            },
            success: function () {
                self.ui.change(posts_to_send.length);
                self.duplicatedIDs = self.duplicatedIDs.concat(posts_to_send);
