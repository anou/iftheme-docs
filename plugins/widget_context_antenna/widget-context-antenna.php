  ...
  function display_widget_context($args = array(), $wid = null) {
    ...
    if(function_exists('multi_antennas')){
      ...
      if($multi){
        ...
        $out .= $this->make_simple_checkbox_antenna($options, $wid, 'country', NULL, __('Country Homepage', 'wca'));
        $out .= '</p></div>';
      }
    }

    $out .= '<div class="wl-header"><h5>' . __('Antennas Context:', 'wca') . '</h5>';
    $out .= '<p class="wl-visibility wl-antennas">';
    // multi-antennas
    if( !isset($antennas['aid']) ) {
      foreach($antennas as $a => $vals) {
        $out .= $this->make_simple_checkbox_antenna($options, $wid, $groupA, $vals['aid'], get_cat_name( $vals['aid'] ));
                      // make_simple_checkbox_antenna($options, $prefix, $id, $fieldname = null, $label)
      }
    } // simple antenna
    else {
...
//    $out .= $this->make_simple_checkbox($options, $wid, $group, 'is_front_page', __('Front Page'));
